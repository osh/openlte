...
%
% Function:    lte_prach_detector
% Description: Detects PRACHs
% Inputs:      x_u               - Sequence of all of the root preambles potentially present
%              prach_bb          - PRACH slot(s) baseband I/Q
%              root_seq_idx      - Logical root sequence index from SIB2
...
% Outputs:     det_preamble - Detected preambles
...
function [det_preamble, det_ta] = lte_prach_detector(x_u, prach_bb, root_seq_idx, pre_format, zczc, hs_flag, prach_freq_offset, N_ul_rb)
    ...
    if(pre_format >= 0 && pre_format <= 3)
        u = table_5_7_2_4(root_seq_idx+1);
    else
        u = table_5_7_2_5(root_seq_idx+1);
    endif
    ...
    if(hs_flag == 0)
        ...
        if(d_u >= N_cs && d_u < N_zc/3)
            ...
            N_new_RA_shift = max(floor((N_zc - 2*d_u - N_RA_group*d_start)/N_cs), 0);
        else
    ...
    % Convert PRACH baseband to the frequency domain
    ...
    for(n=1:N_roots)
        ...
        corr_vec(n,:) = ifft(ifftshift(tmp_corr));
    endfor

    % Find the max
    % FIXME: Only supporting detection of 1 preamble
    max_val    = 0;
    max_root   = 0;
    max_offset = 0;
    ave_val    = 0;
